...
|---|---|---|---|---|---|
| 18 | P1 | **A restart left last run's effects in the new mission.** A beat still in flight kept running after `startMission()`, so the turn-3 breach charge from the previous run lit a fire in the new one. `Director` now carries a `runId`, bumped on reset, and every awaited beat bails if it belongs to a dead run. | Director | Nikhil | DONE |
| 17 | P1 | **`gsap.delayedCall` cannot pace a cinematic in this project.** `main.js` disables lag smoothing on purpose, so one long frame advances every scheduled callback at once — a 3 s frame fired a 3 s sequence in a single tick, and the vehicle-loss sequence played instantly. Paced on `Pause.wait` instead, like the Director's own beats: real time at any frame rate, and it pauses with the pause menu. **Time nothing else with `delayedCall`.** | FX / anything sequenced | Nikhil | DONE |
| 16 | P1 | **The drone reported before it had looked.** A recon sortie's log line, reveal and contacts all landed while the aircraft was still on the pad, which made the sortie decorative. The turn now waits for the scan to finish before any finding lands. Destinations come from mission data (`turn.recon` / `outcome.site`), not from a turn-number table in the renderer. Covered by `npm run fx`. | Drone / Director | Nikhil | DONE |
