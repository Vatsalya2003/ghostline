...
//
// ON TOP OF THE BODIES, TWO FLAT MARKS PER PERSON: a ring on the floor and a
// beacon over the head, RED for armed and GREEN for a hostage. The figures
// carry the identification honestly — silhouette, posture, restraint — but a
// person is 1.8 units tall in a compound full of crates, walls and doorways,
// and the player is at tactical zoom looking at somebody's back half the
// time. The marks are the index, not the argument: they say WHERE and WHICH,
// the body says WHY. The beacon draws with depth testing off, so a contact
// the player has already been told about never gets lost behind a wall.
//
// Amber stays reserved. It is the unresolved marker and nothing else, which
// is why an unclassified contact keeps its dashed amber ring and gets no
// beacon at all — see unresolved() and the dashed branch of marker().
...
//
// A CLASSIFIED contact gets a fat, fully opaque band — red for armed, green
// for a hostage — because this is the one ring the player is meant to read
// across the room. The DASHED amber one is untouched: it is the unresolved
// marker, and it is quiet on purpose.
...
const KIND = {
  hostile: { armed: true, ring: 0xff3b30, beacon: 0xff3b30 },
  civilian: { armed: false, ring: 0x3cf07a, beacon: 0x3cf07a },
};
...
export class Actors {
  constructor(scene, specs = []) {
    ...
    for (const spec of specs) {
      ...
      this.byId.set(spec.id, {
        spec, holder, body, ring, mark, variant, jitter,
  ...
  resolve(id, kind = 'civilian', pose = 'crouch') {
    ...
    a.holder.add(a.ring);
    // The reveal names the contact, so it gets the mark that goes with the
    // name — a beacon in the colour of whatever it turned out to be.
  ...
  // Two things move every frame: the beacons bob, so a contact never reads as
  // a decal stuck to the scenery, and unresolved contacts pulse, so an
  // unclassified return never sits there looking like a settled fact.
  update(t) {
    for (const a of this.byId.values()) {
