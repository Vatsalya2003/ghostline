...
export function resolveSite({ mission, turn, outcome, level } = {}) {
  ...
  // The mission named coordinates outright for THIS choice. Outcome-level data
  // beats the turn's default recon zone — see Level.reconTarget, where the same
  // rule stops a shared table sending the depot's aircraft to Dry Creek.
  const literal = outcome?.reveal;
  if (literal && typeof literal === 'object'
      && Number.isFinite(literal.x) && Number.isFinite(literal.z)) {
    return record(literal.key || 'reveal', literal, 'mission-data');
  }
