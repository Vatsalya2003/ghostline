...
// The tactical map reads the live game — squad, drone, hazards and the fog's
// own memory — rather than being told about it. Its gazetteer is passed in:
// PLACES is Dry Creek's ground and would be nonsense on the seabed, so a
// mission that declares its own `sites` gets those instead.
const tacticalMap = new TacticalMap({
  ...
  state,
  hazards: fx.hazards,
  places: { ...(mission1.environment === 'undersea' ? {} : PLACES), ...(mission1.sites || {}) },
  objectivePlaces: OBJECTIVE_PLACE,
  mission: mission1,
});
...
window.OP = { state, turnManager, director, squad, camera, scene, fog, fx, screenFX, input, pauseMenu, audio, soundscape, map: tacticalMap, ui, mission: mission1, startMission };
